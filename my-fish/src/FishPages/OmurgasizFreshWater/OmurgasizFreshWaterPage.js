...
const FreshWaterOmurgasiz = () => {
    ...
    useMemo(() => {
        axios.get('http://localhost:8081/omurgasiz/getKind/TATLI')
            ...
            <div className={"sub-main"}>
                ...
                <div className={"kind-cards"}>
                    {currentPosts.map(salt => (
                        <Card className={"cards"} style={{width: '18rem'}} key={salt.id}>
                            <Card.Img variant="top"  src={`data:image/${salt.format};base64,${salt.imgUrl}`}  className="card-img"/>
...
export default FreshWaterOmurgasiz;